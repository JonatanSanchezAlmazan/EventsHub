...
export async function Events(params) {
  ...
  if (user.rol === 'owner') {
    filter = Filter({ isAdmin: true, content: contentEvents });
  } else {
    filter = Filter({ content: contentEvents });
  }

  sectionEvents.classList.add('container', 'flex', 'flex-col', 'gap-5', 'pt-10', 'pb-10', 'relative');
  ...
  contentEvents.id = 'contentEvents';
  ...
  const events = await getEvents({ title: '', direction: '', category: '' });
